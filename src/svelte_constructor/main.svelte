<script>
	...
	$effect(() => {
		console.log('new templateToRealValue length =', templateToRealValue.length);
	});
	$effect(() => {
		...
		for (let i = 0; i < templateToRealValue.length; ++i) {
			...
			if (t.previewField) {
				...
				if (t.optionField.currentOption.sys_id || t.optionField.currentOption.isEnumerationColumn) {
...
<RenderDocx templateFields={templateToRealValue} templateTableColumns={svelteDetectedTables}></RenderDocx>
...
<div style="margin-right: 0; width: 250px;">
	<ReferenceField condition="" table="itam_task_docx_template" nameOfDisplayByRefColumn="ITAM Docx template to use" {popupBoxRef}></ReferenceField>

	<!-- <Dropdown fieldTitle="Existing template to use" optionField={configBeingModified} optionSource={getExistingDocxTemplateRecords} onBeforeOptionSelectCallback={onSelectExistingDocxTemplateClick}
	></Dropdown> -->
</div>

<div style="margin-top: 50px; grid-column:1/3;" style:display={selectTaskField.isVisible ? 'unset' : 'none'}>
	<Dropdown
						...
						<div style="display: flex; flex-direction: row; width: 900px; gap: 50px; align-items: center;">
							<div style="flex: 1 1 auto; max-width: 250px;">
								<div class="src-components-dynamicForms-view-fieldWrapper-___styles-module__Field___BH07L" data-test="field-Purchase documentation">
									...
									<MyInput binding={column.template} readonly={true}></MyInput>
								</div>
							</div>

							{#if column.optionField.currentOption.isScripted}
								<Dropdown optionField={column.optionField} optionSource={table.optionField.currentOption.scripts} fieldTitle="Script mapping"
								></Dropdown>
							{:else if !column.optionField.currentOption.isEnumerationColumn}
								<Dropdown optionField={column.optionField} optionSource={table.optionField.currentOption.columns} fieldTitle="Related list column"
								></Dropdown>
							{/if}

							<Checkbox
								title="Enumeration column"
								checked={column.optionField.currentOption.isEnumerationColumn}
								onClickCheckbox={() => onEnumerationOptionClick(column)}
							></Checkbox>
							<Checkbox
								title="Scripted option"
								checked={column.optionField.currentOption.isScripted}
			...
			<div style="display: flex; flex-direction: row; gap: 50px;">
				...
				<div style="">
					<div class="src-components-dynamicForms-view-fieldWrapper-___styles-module__Field___BH07L" data-test="field-Purchase documentation">
						...
						<MyInput binding={pair.originalValue} readonly={true}></MyInput>
					</div>
				</div>

				{#if pair.optionField.currentOption.isScripted}
					<Dropdown optionField={pair.optionField} optionSource={selectTaskField.currentOption.scripts} fieldTitle="Script mapping"></Dropdown>
				{:else}
					<Dropdown optionField={pair.optionField} optionSource={selectTaskField.currentOption.columns} fieldTitle="Task column to map"></Dropdown>
				{/if}

				<Checkbox title="Scripted option" onClickCheckbox={() => onSciptedOptionClick(pair)} checked={pair.optionField.currentOption.isScripted}
				></Checkbox>
			</div>
		{/each}
	</div>
